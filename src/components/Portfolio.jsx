...
const LazyImage = ({ src, alt, isFirst }) => {
  ...
  return (
    <div className="relative w-full h-auto overflow-hidden rounded-xl">
      ...
      {!loaded && (
        <div className="absolute inset-0 bg-gradient-to-br from-gray-300 to-gray-400 dark:from-gray-700 dark:to-gray-800 animate-pulse rounded-xl" />
      )}

      <img
        src={src}
        alt={alt}
        loading={isFirst ? 'eager' : 'lazy'}           // ✅ First image loads eagerly, rest lazy
        fetchpriority={isFirst ? 'high' : 'low'}       // ✅ Prioritize hero image
        decoding="async"                               // ✅ Non-blocking decode
        width={800}                                    // ✅ Prevents layout shift
        height={500}                                   // ✅ Prevents layout shift
        onLoad={() => setLoaded(true)}                 // ✅ Remove blur when loaded
        className={`
                ...
                >
                  <div className="flex flex-col lg:flex-row h-full">
                    ...
                    <div className={`lg:w-3/5 relative overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900 p-8 lg:p-12 flex items-center justify-center ${!isEven ? 'lg:order-last' : ''}`}>
                      ...
                      {/* ✅ Replace plain <img> with optimized LazyImage */}
                      <motion.div
                        id={theme}
                        className={`
                      ...
                      >
                        <LazyImage
                          src={project.img}
                          alt={project.title}
                          isFirst={index === 0}   // ✅ Only first image loads eagerly
                        />
                      </motion.div>
                    </div>
                    ...
                    <div className="lg:w-2/4 p-8 lg:p-12 flex flex-col justify-center relative z-20">
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <span className="text-green-500 font-bold tracking-widest uppercase text-xs">
                          Project {index + 1}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2 mb-10">
                        {project.tech.map((tech, i) => (
                          <span
                            key={i}
                            className={`px-4 py-1.5 text-xs font-bold rounded-full border shadow-sm ${isLight
